...
1. 仓库**保持 private**，插件更新走页面上的 git 手动更新，不发布、不进 dshmarket
   （见需求 3）。此前定的「发布渠道：GitHub Release」因此作废，仅在将来决定公开
   仓库时才需要重拾。
2. 需求 5 的「增量」：**接受「按更小日期窗口重取」的定义**，MES 不支持按修改
   时间过滤，真正的差量同步做不了。
3. 页面默认数据源：**读缓存**，且页面必须显示数据更新至什么时间。
4. 同步窗口**自动扩展到覆盖已缓存的一切**，配「清空缓存」重置范围。
5. 联网动作一律只在用户点击时发生：打开页面既不查 mes 更新，也不查插件更新。
