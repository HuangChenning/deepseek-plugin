...
test('the settings view explains the fixed public update source', () => {
  const html = renderPage()
  const settingsStart = html.indexOf('<section id="settings-view"')
  const settingsView = html.slice(settingsStart, html.indexOf('</section>', settingsStart))

  assert.match(settingsView, /origin/)
  assert.match(settingsView, /官方 HTTPS/)
  assert.match(settingsView, /官方 <code>main<\/code>/)
  assert.doesNotMatch(html, /github-auth|GitHub 授权|ssh-agent/u)
})
